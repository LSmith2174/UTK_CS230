...
//This part of the project uses what we made in the first three parts of the pipeline, 
//fetch, decode, and execute and builds upon it. The memory stage builds upon load and store,
//taking what the ALU did in the execute stage and actually reading or writing values. This
//code supports LB, LBU, LH, LHU, LW, LWU, LD as well as SB, SH, SW, and SD. Once the memory()
//function runs, it tests to see if the instruction is a load or store. Then if a store, uses
//the function memory_write to take the execute result, and the right_val and puts the right_val
//into the location given by the execute result. If a load, it uses the function memory_read
//and gets the value at the location given by the execute result. This is the fourth stage of
//the pipline and is nearly the completion of this project. 
...
class Machine {
   ...
   void memory_write(int64_t address, T value) {
    ...
    mDO.rd        = (mFO.instruction >> 7) & 0x1f;
    mDO.funct3    = (mFO.instruction >> 12) & 7;
    mDO.offset    = get_xreg(mFO.instruction >> 15); // get_xreg truncates for us
    mDO.right_val = get_xreg(mFO.instruction >> 20);
    mDO.left_val  = sign_extend((((mFO.instruction >> 7) & 0x1f) << 0) |
